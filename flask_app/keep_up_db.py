...
def get_times_sleeping_for_driver():
    ...
    for driver in Drivers.objects:
        list_of_drivers_sleeping.append({'y': len(driver.data_drivers_sleeping.times), 'label': driver.name})
...
def get_sleep_by_day():
    list_of_day = []
    for i in range(7):
        list_of_day.append({'y': 0, 'label': i})
    for driver in Drivers.objects:
        for time_of_sleep in driver.data_drivers_sleeping.times:
            day = ((time_of_sleep.weekday() + 1) % 7)
            list_of_day[day]['y'] += 1
    dict_of_days = {'data': list_of_day}
    return dict_of_days
